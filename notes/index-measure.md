...
- ⚠ **【2026-09-27 訂正】「収束する」は誤りだった。40ページ走査は全件の列挙ではなく標本である。**
  同日に2回走査したところ、40ページ走査（44件）に出なかった `socket-taihen-size` が、
  2回目（18ページで停止）に出た。**前日比の増減は、この標本の揺れと区別できない。**
  実測: 09-26→09-27 で **5件増え・6件消えた**（差し引き −1）。**件数の差で「増えた/減った」と書かない**
- ⚠ **連続で走査すると打ち切られる。**同じ回の2回目は18ページ、3回目は1ページで `empty-page` 停止した。
  **1回の稼働で走査は1回だけ。**`stopped` が `cap` 以外なら、その値は下限としても弱い
